...
function parsePassages(story, options) {
    var ignoreComments = _.has(options, "ignoreComments") ? options.ignoreComments : false,
        ...
        linkFormat = _.has(options, "linkFormat") ? options.linkFormat : null;
    _.each(story.passages, function(passage) {
        ...
        passage.text = parseLinks(passage, story, linkFormat);
        if (hasCustomTags) {
...
function parseLinks(passage, story, linkFormat) {
    var linkFormatTemplate = null;
    if (linkFormat !== null && _.isString(linkFormat)) {
        linkFormatTemplate = _.template(linkFormat);
    }
    return passage.text.replace(/\[\[(.*?)\]\]/g, function(match, innerText) {
        ...
        if (linkFormatTemplate !== null) {
            // apply custom template string
            return linkFormatTemplate(link);
        } else if (linkFormat !== null && _.isFunction(linkFormat)) {
            return linkFormat(link);
        }
